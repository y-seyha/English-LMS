      ...
      <div className="mx-auto flex h-16 max-w-[1120px] items-center gap-8 px-6 max-lg:justify-between">
        ...
        <nav
          className={`flex flex-col
            ...
            max-lg:absolute max-lg:left-0 max-lg:right-0 max-lg:top-16 max-lg:z-60 max-lg:gap-1 max-lg:border-b max-lg:border-[--border] max-lg:bg-[--card] max-lg:p-3 max-lg:shadow-lg max-lg:overflow-hidden max-lg:transition-all max-lg:duration-300
        ...
        >
          {navItems.map(item => (
            <NavLink
              ...
              to={item.to}
              className={({ isActive }) =>
                `block rounded-lg px-3 py-2 text-[0.9375rem] font-medium text-muted transition-all hover:bg-black/4 hover:text-[--foreground] dark:hover:bg-white/6 ${isActive ? 'bg-blue-100 text-blue-700 dark:bg-sky-900/40 dark:text-sky-300' : ''}`
              }
              end={item.to === '/'}
