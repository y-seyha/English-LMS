    ...
    <div className={`mb-4 rounded-xl border bg-[--card] p-6 shadow-[--shadow] transition-shadow duration-200 ${showResult ? (isCorrect ? 'border-l-3 border-l-emerald-500 dark:border-l-emerald-400' : 'border-l-3 border-l-red-500 dark:border-l-red-400') : 'border-l-3 border-l-blue-600 border-[--border] dark:border-l-sky-500'}`}>
      ...
      {showResult && (
        <div className="mt-4 flex flex-col gap-3">
          <div className={`rounded-lg border px-4 py-3 text-sm ${isCorrect ? 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950/20 dark:text-emerald-400' : 'border-red-200 bg-red-50 text-red-600 dark:border-red-800 dark:bg-red-950/20 dark:text-red-400'}`}>
            <p>
              {isCorrect ? '✓ ' : '✗ '}
          ...
          {!isCorrect && (
            <button
              className="inline-flex w-fit items-center gap-2 rounded-lg border border-[--border] px-4 py-2 text-sm font-medium transition-all hover:bg-black/4 dark:hover:bg-white/6"
